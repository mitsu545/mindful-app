...
> 2026-09-15 更新：当初はタイマー画面に「アンカー文言」（足の裏の感覚／坐骨の重み／お腹の上下、の3種からランダム表示）を出す予定だったが、実際に使ってみて不要と判断し廃止。タイマー画面には残り時間と雑念ボタンのみを表示する。

### 4-3. 雑念ボタン（ラベリング）

`考え事` / `不安` / `タスク` の3つ。

> 2026-09-15 更新：「計画」は「仕事・日常のタスク／直近でやらなきゃいけないこと」の方が実態に合うため「タスク」に変更。
...
| **上下3分割の巨大ボタン**として配置（上＝考え事／中＝不安／下＝タスク） | 目を閉じたまま指の位置感覚だけで押し分けられるようにするため |
...
| **メール通知（未実行検知）** | 未実行の検知（フェールセーフ） | GAS が毎晩21時にチェック、**その日の記録が0件なら送信** |
| **メール通知（ランダムリマインド）** | 気分入力を促す（If-Then の If の代替） | GAS が毎時トリガーで実行、**8〜21時の間だけ、確率的にランダムなタイミングで送信**（2026-09-16追加） |
...
> 2026-09-16 更新：本物のプッシュ通知（アプリを開いていなくても端末にポップアップが出るもの）は、GAS＋静的サイトの構成では実装できない（9章の通り、専用サーバが必要）。そのため「ランダムなタイミングで気分入力を促す」仕組みは、既存のメール通知の仕組みを流用して実現した。
